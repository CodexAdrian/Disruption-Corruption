...
* Rename the main package to be the same as `${mod_base_package}.${mod_id}`, e.g. `me.codexadrian.dandc`

### Using mixins?
* Delete `build.gradle`
* Rename `mixinbuild.gradle` to `build.gradle`
* Rename `examplemod.mixins.json` to start with your mod id
* Add mixins under the `mixin` package inside your base package, e.g. `me.codexadrian.dandc.mixin`
